...
class NetworkTaskTests: XCTestCase {
    ...
    func testUnsubscribeOnEventMethod() {
        let expectation: XCTestExpectation = self.expectation(description: "unsubscribe")
        let eventId = 7655
        
        registerUser(expectation: expectation) {
            NetworkService.sharedService.subscribeOnEvent(eventUUID: eventId, success: { (_) in
                
                NetworkService.sharedService.unsubscribeFromEvent(eventUUID: eventId, success: { (responseData) in
