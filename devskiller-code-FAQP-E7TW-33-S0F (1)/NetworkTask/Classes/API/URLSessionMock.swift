...
class URLSessionMock : URLSession {
    ...
    private let unsubFromEventEndpoint = "/leave"
    ...
    private func handleRequest(request: URLRequest) -> (data: Data?, error: Error?) {
        ...
        if let url = request.url {
            ...
            case let p where isUnSubFromEventEndpoint(urlPath: p):
                return UserEndpointHandler().handleUnsubscription(request: request)
            default:
    ...
    private func isUnSubFromEventEndpoint(urlPath: String) -> Bool {
        return urlPath.hasPrefix(apiEndpoint + eventEndpoint)
            && urlPath.hasSuffix(unsubFromEventEndpoint)
    }
}
